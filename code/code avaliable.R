...
length(unique(example$RSID_locus))

### Identify unique loci
cuts <- unique(example$RSID_locus)
...
### Process each locus
locus <- 1
dat <- example[example$RSID_locus == cuts[locus],]
...
r1 <- r1[,c(1,3)]
all <- example
length(unique(all$RSID_locus))
a <- unique(all$RSID_locus)

for (i in 1:length(a)) {
  i=1
  locus <- all[all$RSID_locus == a[i],]
...
length(unique(info$RSID_locus))
length(unique(info$RSID))
names(info)
snp <- info[,c(1,10,21:23)]
...
length(unique(snp_cree_overlap$RSID_locus))
func <- snp_cree_overlap
func<-func[,c(1:6)]
names(func)
func <-merge(func,info,by=c("RSID_locus","RSID"))
length(unique(func$RSID))
length(unique(func$RSID_locus))
...
length(unique(info_reg$RSID_locus))
